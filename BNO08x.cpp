...
int_asserted(false)
{
    ...
    imu_spi_config.spics_io_num = -1; //due to esp32 silicon issue, chip select cannot be used with full-duplex mode driver, it must be handled via calls to gpio pins
...
bool BNO08x::initialize()
{
    ...
    {
        uint32_t sw_part_number = ((uint32_t)rx_buffer[7] << 24) | ((uint32_t)rx_buffer[6] << 16) | ((uint32_t)rx_buffer[5] << 8) | ((uint32_t)rx_buffer[4]);
        uint32_t sw_build_number = ((uint32_t)rx_buffer[11] << 24) | ((uint32_t)rx_buffer[10] << 16) | ((uint32_t)rx_buffer[9] << 8) | ((uint32_t)rx_buffer[8]);
 ...
 * 
 * @return The reason for the most recent recent reset ( 1 = POR (power on reset), 2 = internal reset, 3 = watchdog timer, 4 = external reset 5 = other)
 */
 ...
 * Waits for accuracy of returned quaternions and magnetic field vectors to be high, then saves calibration data and returns. 
 * 
 * @return void, nothing to return
 */
bool BNO08x::run_full_calibration_routine()
{
    ...
    while(1)
    {
        if(data_available())
        {
            ...
            ESP_LOGI(TAG, "Quaternion Rotation Vector: i: %.3f j: %.3f k: %.3f, real: %.3f, accuracy: %d", quat_I, quat_J, quat_K, quat_real, quat_accuracy);
...
uint16_t BNO08x::get_readings()
{
	...
	{
        if(imu_config.debug_en)
		    ESP_LOGE(TAG, "SHTP Header RX'd: 0x%X 0x%X 0x%X 0x%X", packet_header_rx[0], packet_header_rx[1], packet_header_rx[2], packet_header_rx[3]);
		...
		if (packet_header_rx[2] == CHANNEL_REPORTS && rx_buffer[0] == SHTP_REPORT_BASE_TIMESTAMP)
		{
            ...
            return parse_input_report(); //This will update the rawAccelX, etc variables depending on which feature report is found
		}
...
uint16_t BNO08x::parse_input_report()
{
    ...
    time_stamp = ((uint32_t)rx_buffer[4] << (8 * 3)) | ((uint32_t)rx_buffer[3] << (8 * 2)) | ((uint32_t)rx_buffer[2] << (8 * 1)) | ((uint32_t)rx_buffer[1] << (8 * 0));
    ...
    switch(rx_buffer[5])
    {
        ...
        case SENSOR_REPORTID_PERSONAL_ACTIVITY_CLASSIFIER:
            ...
            for (i = 0; i < 9; i++)					   //Hardcoded to max of 9. TODO - bring in array size
                activity_confidences[i] = rx_buffer[5 + 6 + i]; //5 bytes of timestamp, byte 6 is first confidence byte
        break; 
 ...
 * @param rotation_vector_basis Which rotation vector type to zero axes can be TARE_ROTATION_VECTOR, TARE_GAME_ROTATION_VECTOR, TARE_GEOMAGNETIC_ROTATION_VECTOR, etc..
 * @return void, nothing to return
 */
...
/**
 * @brief Converts a register value to a float using its associated Q point. (See https://en.wikipedia.org/wiki/Q_(number_format))
 * 
...
/**
 * @brief Get full rotational velocity without drift compensation (units in Rad/s). An estimate of drift is given but not applied.
 * 
 * @param x Reference variable to save X axis angular velocity 
 * @param y Reference variable to save Y axis angular velocity 
 * @param z Reference variable to save Z axis angular velocity 
...
void BNO08x::print_header()
{
    ...
    ESP_LOGI(TAG, "SHTP Header:\n\r"
        ...
        "                       Sequence Number: %d\n\r"
        "                       Channel Type: %s\n\r",
        (int)packet_header_rx[0],
        (int)packet_header_rx[1],
        (int)packet_header_rx[2],
        (int)packet_header_rx[3],
        (int)(packet_length_rx+4),
        (int)packet_header_rx[2],
        (int)packet_header_rx[3], 
        ...
        (packet_header_rx[2] == 5) ? "Gyro-vector" : "Unknown");
}
...
/**
 * @brief Reads meta data word from BNO08x FRS (flash record system) given the record ID and word number. (See Ref. Manual 5.1 & 6.3.7)
 * 
 * Note that Q points from the data sheet can be used as well, using the ones stored in flash is optional. 
 * 
...
/**
 * @brief Requests meta data from BNO08x FRS (flash record system) given the record ID. Contains Q points and other info. (See Ref. Manual 5.1 & 6.3.6)
 * 
 * Note that Q points from the data sheet can be used as well, using the ones stored in flash is optional. 
 * 
 * @param record_ID Which record ID/ sensor to request meta data from. 
...
/**
 * @brief Read meta data from BNO08x FRS (flash record system) given the record ID. Contains Q points and other info. (See Ref. Manual 5.1 & 6.3.7)
 * 
 * Note that Q points from the data sheet can be used as well, using the ones stored in flash is optional. 
 * 
 * @param record_ID Which record ID/ sensor to request meta data from. 
...
/**
 * @brief Queues a packet containing a command with a request for sensor reports, reported periodically. (See Ref. Manual 6.5.4)
 * 
 ...
 * @param rotation_vector_basis Which rotation vector type to zero axes of, BNO08x saves seperate data for Rotation Vector, Gaming Rotation Vector, etc..)
 * 
 * @return void, nothing to return
 */
...
void IRAM_ATTR BNO08x::hint_handler(void *arg)
{
    ...
    BNO08x *imu = (BNO08x *)arg; //cast argument received by gpio_isr_handler_add ("this" pointer to imu object created by constructor call)

    gpio_intr_disable(imu->imu_config.io_int); //disable interrupts
    vTaskNotifyGiveFromISR(imu->spi_task_hdl, &xHighPriorityTaskWoken); //notify SPI task BNO08x is ready for servicing
